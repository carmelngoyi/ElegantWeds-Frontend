...
const Bookings = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const response = await axios.post(`${API_BASE}/bookings`, formData);
        ...
        if (err.code === 'ERR_NETWORK') {
            errorMsg = 'Connection Error: Cannot reach the backend API. Check API_BASE URL and CORS settings.';
        } else {
            errorMsg = err.response?.data?.error || 'Failed to save booking.';
        }
        ...
        setTimeout(() => setSubmissionStatus(null), 5000); // Clear status message after 5 seconds
    }
  };
          ...
          <section className="booking-form-section">
            <form className="booking-form-box" onSubmit={handleSubmit}>
              ...
              {submissionStatus === 'success' && (
                  <p className="status-message status-success">Thank you for booking your enquiry! We will be in touch soon.</p>
              )}
              {submissionStatus === 'error' && (
                  <p className="status-message status-error">Submission failed. Please check your form details and API connection.</p>
              )}
